...
The system is instrumented with [OpenTelemetry](https://opentelemetry.io/) for distributed tracing. This provides deep visibility into the behavior of the system, allowing us to debug and monitor it effectively in a production environment.

### Tracing Spans

Tracing spans are created for each of the following operations:

-   **Orchestrator:** A top-level span is created for each task that the orchestrator receives.
-   **SupervisorAgent:** A nested span is created when the supervisor routes a task to a worker.
-   **ExecutorAgent:** Another level of nesting is created when the executor runs a task.
-   **Tools:** The deepest level of spans are created when a tool is executed.

This creates a detailed, hierarchical trace for every task, which can be visualized in a tracing backend like Jaeger.
